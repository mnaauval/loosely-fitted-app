...
const Modal = ({ showModal, setShowModal, productId }) => {
  ...
  return (
    <>
      {/* {showModal ? ( */}
      <div className="overflow-y-auto flex items-center justify-center fixed inset-0 z-50 bg-black/80" ref={modalRef} onClick={closeModal}>
        {/* Modal Wrapper */}
        <div className="relative inline-block align-bottom rounded-lg text-left overflow-hidden shadow-lg transform transition-all sm:my-6 sm:align-middle sm:max-w-[800px] sm:w-auto ">
          ...
          <div className="w-full relative flex items-center bg-white px-4 pt-14 pb-8 overflow-hidden sm:px-6 sm:pt-8 md:p-6 lg:p-8">
            {/* Button X */}
            <button type="button" className="absolute top-4 right-4 text-gray-400 hover:text-gray-500 sm:top-8 sm:right-6 md:top-6 md:right-6 lg:top-8 lg:right-8" onClick={() => setShowModal((prev) => !prev)}>
          ...
          <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button type="button" className="w-full inline-flex justify-center rounded-md border border-gray-700 shadow-sm px-4 py-2 bg-transparent text-base font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm" onClick={() => setShowModal((prev) => !prev)}>
              OK
            </button>
          </div>
        </div>
